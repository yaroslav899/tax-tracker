...
export const DividendChart = ({ dividends }: DividendChartProps) => {
  ...
  const { chartData, totalPln, totalUsd } = useMemo(() => {
    ...
    const chartData = MONTHS.map((month, i) => ({
      month,
      pln: Math.round(monthlyPln[i] * 100) / 100,
    }))

    const totalPln = Math.round(filtered.reduce((sum, d) => sum + (d.valuePln ?? 0), 0) * 100) / 100
    const totalUsd = Math.round(filtered.reduce((sum, d) => sum + d.value, 0) * 100) / 100

    return { chartData, totalPln, totalUsd }
