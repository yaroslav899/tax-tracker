...
const StatRow = ({
  ...
  <div className="flex flex-col gap-1.5 py-3 border-b border-white/5 last:border-0">
    ...
    <div className="flex items-baseline gap-3">
      <span
        className={`text-sm font-semibold ${highlight ? 'text-yellow-400' : negative ? 'text-red-400' : 'text-white'}`}
      >
        {negative ? '−' : ''}
      ...
      <span
        className={`text-sm font-medium ${pln != null ? (highlight ? 'text-yellow-300' : negative ? 'text-red-300' : 'text-gray-300') : 'text-gray-600'}`}
      >
...
export const TaxSummaryWidget = ({ dividends }: TaxSummaryWidgetProps) => {
  ...
  const result = useMemo(() => {
    if (!calculated || !effectiveYear) return null
    const filtered = dividends.filter((d) => d.date.split('/')[2] === effectiveYear)
    ...
    const taxDuePln =
      taxBasePln != null && withholdingPln != null
  ...
  }, [calculated, effectiveYear, dividends])
    ...
    <div className="w-72 shrink-0">
      <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        ...
        <div className="px-5 py-2">
          ...
          {calculated && !result && (
            <div className="flex items-center justify-center py-8">
              <span className="text-gray-500 text-xs text-center">No data for {effectiveYear}</span>
            </div>
          )}
          {result && (
            <div>
        ...
        <div className="px-5 pb-5 pt-3">
          <Button onClick={() => setCalculated(true)} className="w-full justify-center">
            {t('taxWidget.calculate')} {effectiveYear}
          </Button>
        </div>
      </div>
    </div>
